...
- **文件浏览器** — 通过面包屑导航浏览书库
- **上传文件和文件夹** — 拖放或点击即可上传书籍，也可一次上传整个文件夹
- **下载文件** — 一键将文件保存到手机
- **创建文件夹** — 将书库整理为不同目录
...
- **上传** — 点击顶栏的 **上传** 按钮，然后选择文件或将文件拖到上传区域。支持同时上传多个文件。若要上传整个文件夹，请使用 **选择文件夹** 或直接拖入文件夹——子文件夹会在设备上重建。
...
- 上传的文件夹会完整保存，但其中非书籍、文档或图片类的文件（例如插件的 `.lua` 文件）在关闭安全模式前不会显示
