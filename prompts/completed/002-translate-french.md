<objective>
Translate all strings in the FileSync KOReader plugin from English to French (fr).
...
3. Create `./filesync/i18n/fr.po` with ALL msgid entries translated to French
...
6. Use standard metropolitan French with natural, user-friendly phrasing
7. For the header block, set `"Language: fr\n"` and update the comment to say "French translations"
8. The section comment `# Web UI translations` should be preserved as-is
9. For short UI labels (like "m ago", "h ago", "d ago"), use appropriately concise French abbreviations
</requirements>

<output>
Save the completed translation to: `./filesync/i18n/fr.po`
</output>

<verification>
Before saving, verify:
- Every msgid from en.po has a corresponding entry in fr.po
