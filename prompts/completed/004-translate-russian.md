<objective>
Translate all strings in the FileSync KOReader plugin from English to Russian (ru).
...
3. Create `./filesync/i18n/ru.po` with ALL msgid entries translated to Russian
...
6. Use natural, user-friendly Russian phrasing
7. For the header block, set `"Language: ru\n"` and update the comment to say "Russian translations"
8. The section comment `# Web UI translations` should be preserved as-is
9. For short UI labels (like "m ago", "h ago", "d ago"), use appropriately concise Russian abbreviations
</requirements>

<output>
Save the completed translation to: `./filesync/i18n/ru.po`
</output>

<verification>
Before saving, verify:
- Every msgid from en.po has a corresponding entry in ru.po
