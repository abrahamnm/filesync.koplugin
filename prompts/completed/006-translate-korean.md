<objective>
Translate all strings in the FileSync KOReader plugin from English to Korean (ko).
...
3. Create `./filesync/i18n/ko.po` with ALL msgid entries translated to Korean
...
6. Use natural, user-friendly Korean phrasing suitable for a software UI (use 합니다/합쇼 polite form for messages, concise labels for UI elements)
7. For the header block, set `"Language: ko\n"` and update the comment to say "Korean translations"
8. The section comment `# Web UI translations` should be preserved as-is
9. For short UI labels (like "m ago", "h ago", "d ago"), use appropriately concise Korean expressions (e.g., 분 전, 시간 전, 일 전)
</requirements>

<output>
Save the completed translation to: `./filesync/i18n/ko.po`
</output>

<verification>
Before saving, verify:
- Every msgid from en.po has a corresponding entry in ko.po
